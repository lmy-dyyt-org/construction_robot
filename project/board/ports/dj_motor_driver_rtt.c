...
#define can2_pid10_out motor_can2_speed_pid[10].parameter.out
	...
	/////////////////////////加载输出/////////////////////////////////////
	//注意！！！下面的每个函数里都包含了两个函数，即发送两串指令。。。但是都注释掉了一个，使一次只能发送两串指令。。。后三个id的6020和其他电机只能动一个。
    ...
    set_6020_allGroup(&hcan1,0,0,0,
	0,can1_pid8_out,can1_pid9_out,can1_pid10_out,0);

    set_6020_allGroup(&hcan2,0,0,0,
	0,can2_pid8_out,can2_pid9_out,can2_pid10_out,0);
