...
////////////////////////////////////////////////////////////        
    ...
    */
    ...
    if(draw_points.y > points_01[1].y ) draw_points.y = points_01[1].y ;
    if(draw_points.y < points_23[1].y ) draw_points.y =  points_23[1].y;
    if(draw_points.x < points_12[1].x ) draw_points.x = points_12[1].x;
    if(draw_points.x > points_30[1].x ) draw_points.x = points_30[1].x;
    static int time = 0;
    if(time++%100==0)LOG_I("now step:%s drae_y %f",step==A?"A":step==B?"B":step==C?"C":step==D?"D":"OVER",draw_points.y);
    graphics2corexy(&now_points, &draw_points);
    corexy_absolute_move(&corexy,now_points.x,now_points.y);
