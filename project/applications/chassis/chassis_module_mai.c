...
#define CHSSIS_MAI_A_M (0.218f)
#define CHSSIS_MAI_B_M (0.24f)
...
int module_mai(struct chassis *chassis, const void *output, const void *input)
{
    if (output != NULL)
    {
        ...
        case CHASSIS_POS:
            ...
            data->motor1 = ((chassis->target.pos.x_m + chassis->target.pos.y_m) - (chassis->target.pos.z_rad * CHASSIS_HALF_A_B)) *conversion ;//430
    ...
    if (input != NULL)
    {
        ...
        case CHASSIS_POS:
            ...
            LOG_D("pos get motor1:%f motor2:%f motor3:%f motor4:%f,xm:%f,ym:%f,zrad:%f", data->motor1, data->motor2, data->motor3, data->motor4,chassis->present.pos.x_m,chassis->present.pos.y_m,chassis->present.pos.z_rad);
