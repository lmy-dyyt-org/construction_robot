...
int module_mai(struct chassis *chassis, const void *output, const void *input)
{
    if (output != NULL)
    {
        ...
        case CHASSIS_SPEED:
            ...
            data->motor1 = ((chassis->target.speed.x_m_s + chassis->target.speed.y_m_s)   - (chassis->target.speed.z_rad_s * CHASSIS_A_B)) * CHASSIS_2PIR/60.f;
            data->motor2 = ((chassis->target.speed.x_m_s - chassis->target.speed.y_m_s)   - (chassis->target.speed.z_rad_s * CHASSIS_A_B)) * CHASSIS_2PIR/60.f;
            data->motor3 = ((-chassis->target.speed.x_m_s - chassis->target.speed.y_m_s)  - (chassis->target.speed.z_rad_s * CHASSIS_A_B)) * CHASSIS_2PIR/60.f;
            data->motor4 = ((-chassis->target.speed.x_m_s + chassis->target.speed.y_m_s)  - (chassis->target.speed.z_rad_s * CHASSIS_A_B)) * CHASSIS_2PIR/60.f;
            break;
        case CHASSIS_POS:
            // 位置控制
            // 将位置(m)转换为电机数据(rpm/min) 使用车轮半径和车体尺寸
            data->motor1 = ((chassis->target.pos.x_m + chassis->target.pos.y_m) - (chassis->target.pos.z_rad * CHASSIS_HALF_A_B)) *conversion ;
    ...
    if (input != NULL)
    {
        ...
        case CHASSIS_POS:
            ...
            chassis->present.pos.x_m = ((data->motor1 + data->motor2 - data->motor3 - data->motor4) / conversion) / 4.f;
            chassis->present.pos.y_m = ((data->motor1 - data->motor2 - data->motor3 + data->motor4) / conversion) / 4.f;
            chassis->present.pos.z_rad = ((-data->motor1 - data->motor3 - data->motor2 - data->motor4) / conversion) / (4.f * CHASSIS_HALF_A_B);
            break;
        default:
